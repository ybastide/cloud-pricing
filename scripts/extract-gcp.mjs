...
function parseInstanceRow(cells, family, requireMemoryUnit, familyType) {
...
export function extractInstanceRows(
    ...
    const familyType = startHeading.split(' ', 1)[0].replace('-', ' ')
    ...
    for (let i = 0; i < headings.length; i++) {
        ...
        for (let j = 0; j < typeIdxs.length; j++) {
            ...
            rows.push(parseInstanceRow(dataCells.slice(from, to), family, requireMemoryUnit, familyType))
        }
    }
    return rows
}
...
export const INSTANCE_SOURCES = [
    {
        prefix: 'Compute-optimized VM pricing',
        start: 'Compute-optimized pricing',
        end: 'Simulated maintenance event pricing'
    },
    {
        prefix: 'Memory-optimized VM Pricing',
        start: 'Memory-optimized pricing',
        end: 'Tier_1 higher bandwidth network pricing'
    },
    {
    ...
    {
        prefix: 'Storage-optimized VM Pricing',
        start: 'Storage-optimized pricing',
        end: 'Simulated maintenance event pricing'
    },
]

export function splitRows(rows) {
    const familyTypeToFamilySet = {}

    for (const row of rows) {
        if (!(row.familyType in familyTypeToFamilySet)) {
            familyTypeToFamilySet[row.familyType] = new Set()
        }

        familyTypeToFamilySet[row.familyType].add(row.family)
    }

    const familyTypeToFamilyArray = Object.entries(familyTypeToFamilySet).map(
        ([familyType, set]) => ({
            familyType,
            families: Array.from(set),
        }),
    )

    return familyTypeToFamilyArray
}
...
function main() {
    ...
    const familyTypeToFamilyArray = splitRows(instances)
    ...
    writeFileSync('fixtures/gcp/family-type-to-family-array.json', JSON.stringify(familyTypeToFamilyArray, null, 2) + '\n')
